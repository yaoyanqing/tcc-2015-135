...
Dodis et al. \cite{D01} introduced more realistic  source, called Bias-Control Limited (BCL) source, which  realistically generalizes the SV source of Santha and Vazirani  \cite{SV86} and the bit-fixing source of Lichtenstein, Linial, and Saks \cite{LLS89}.  The BCL source  generates a sequence of bits
 $x_1, x_2, \ldots, x_n$, where for $i =1, 2, \ldots, n$, the value  of $x_i$
can depend on  $x_1, x_2, \ldots,  x_{i-1}$ in one of the following two ways: (A) $x_i$ is determined by $x_1, \ldots, x_{i-1}$, but this happens for at most $b$ bits, or (B)  $  \frac{1- \delta}{2}  \leq   \Pr[x_i=1 \mid x_1,  x_2, \ldots, x_{i-1}]  \leq \frac{1+\delta}{2}$, where $ 0 \leq \delta < 1$. (See Definition \ref{DFBCL} for details.)  In particular, when
...
Hence, compared with the SV source, the  BCL source appears much more realistic, especially if the number of interventions $b$ is somewhat moderate.
...
\item   We use a new truncation technique for the design of a finite-precision mechanisms to satisfy this property (see Step 2 of Section 4.1).
...
\begin{definition}
\label{SVCS}
 {\slshape  Let $ \tilde{c} > 1$ be a constant and $\zeta > 0$.  We say that a mechanism M is an interval mechanism (see \cite{DLMV12}) if for all queries $f \in \mathcal{F}$, all
databases $D \in \mathcal{D}$, and all possible outcomes $z \in \mathbb{Z}$, $T \overset{def}{=} T(D,f, z)$, the set $\{ \sum \limits_{i=1}^n r_i \cdot 2^{n-i} \mid r_1\ldots r_n \in T \}$ contains consecutive integers.
