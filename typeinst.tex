...
\begin{remark}
 ...
 In this paper, we assume that the randomness $  \mathbf{r}  $ as input of  the mechanism $M$  is in $ \{0, 1\}^*$.  For a fixed outcome $z$, $M$ needs only a finite number of coins $n \overset{def}{=} \tilde{\tau}(D, f, z)$ to determine whether $M(D, f) = z$.  Therefore,
...
Consider a mechanism $M$ with randomness space $\{0, 1\}^*$.
Denote $ \widetilde{T}(D, f, z)  \overset{def}{=} \{  \mathbf{r} \in \{0, 1\}^* \mid z = M(D, f; \mathbf{r}) \}$ as  the set of all coins  $\mathbf{r} \in \{0, 1\}^*$ such that $M$ outputs $z$ when running on  database $D$, query $f$, and random coins $\mathbf{r}$.   It should be noted that in our model only $n \overset{def}{=} \tilde{\tau} (D, f, z)$ coins need to be sampled to determine if $M(D, f)=z$. Therefore, let $T(D, f, z) \overset{def}{=} \{r \in {0, 1}^n \mid z = M(D, f; r)\}$, without loss of generality,  we can assume that $\widetilde{T}(D, f, z) =T(D, f, z)$.
...
Denote $n \overset{def}{=} \tau (\min(f(D_1), f(D_2)), z)$.
...
Let $ \bar{s}_{y-1}(k-1)$ (resp. $ \bar{s}_{y-1}(k)$) be  $s_{y-1}(k-1)$ (resp. $s_{y-1}(k)$), rounded  to the first $n \overset{def}{=}\tau (y-1, k/\varepsilon)$ bits after the binary point.
