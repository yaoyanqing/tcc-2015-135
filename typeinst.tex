...
\item   A new truncation technique is proposed in the design  finite-precision mechanism to satisfy this property.

\item   Rigorous proofs about differential privacy and accuracy of this kind of mechanism are shown.

\item  While the result of \cite{DY14} implies a {\slshape possible } result about differentially private mechanisms: If there exist $( \mathcal{BCL}(\delta, b, n), \xi)-$differentially private and $(U_n, \rho)$-accurate mechanisms  for the Hammimg weight queries, then $ \rho > \frac{2^{b \cdot \log (1+ \delta) -9}}{\xi}$.  We build {\slshape explicit} such mechanisms and the parameters match the above condition.
...
Dodis et al. \cite{D01} introduced more realistic  source, called Bias-Control Limited (BCL) source, which  realistically generalizes the SV source of Santha and Vazirani  \cite{SV86} and the bit-fixing source of Lichtenstein, Linial, and Saks \cite{LLS89}.  The BCL source  generates
...
However, SV-consistent sampling is useless to BCL source as some strings can't be generated from BCL source, which is crucial to the SV-consistent sampling property. Recently, Dodis and Yao \cite{DY14} have shown an impossibility result for BCL source: when $b \geq  \Omega( \log( \xi \rho )/ \delta)$, it's impossible to achieve  $( \mathcal{BCL}(b, \delta, n), \xi)$-differentially private and $(\mathcal{BCL}(b, \delta, n),  \rho)$-accurate mechanism for Hamming weight queries. In other words, if  there exists a   $( \mathcal{BCL}(b, \delta, n), \xi)$-differentially private and $(\mathcal{BCL}(b, \delta, n),  \rho)$-accurate mechanism for Hamming weight queries, then $b \leq O(\log( \xi \rho )/ \delta)$.  This result gives us a bit hope to
...
on $\mathcal{BCL}(b, \delta, n)$ with $b \neq 0$ can't be an interval one.
...
\item   We use a new truncation technique in the design  finite-precision mechanism to satisfy this property (see Step 2 of Section 4.1).
...
\item  While the result of \cite{DY14} implies that if  there exists a $( \mathcal{BCL}(\delta, b, n), \xi)-$differentially private and $(U_n, \rho)$-accurate mechanism  for the Hammimg weight queries, then the parameters should satisfy $ \rho > \frac{2^{b \cdot \log (1+ \delta) -9}}{\xi}$, we build {\slshape explicit} such mechanisms and the parameters match the above condition (Theorem \ref{spe}).  Thus we have made some progress.
...
We call a family   $ \mathcal{R}$ of distributions over $\{0, 1\}^*$ a source, where $\{0, 1\}^* \overset{def}{=} \bigcup \limits_{ m \in \mathbb{Z}^+ } \{0, 1\}^m = \{0, 1\} \cup \{0, 1\}^2 \cup \ldots  \cup \{0, 1\}^m \ldots$.       Denote $ \mathcal{U} $  as the uniform source, which
 is the set containing only the distribution $U$  on $\{0, 1\}^*$ that samples each bit independently and uniformly at random. For a set $S$, we write $U_S$ to denote the uniform distribution over $S$.  For a distribution or a random variable $R$, let $\mathbf{r} \leftarrow R$
...
Let $ \mathcal{D}$ be the space of all databases.  For simplicity, we only consider the query function $f:  \mathcal{D} \rightarrow \mathbb{Z}$.
 We assume that the randomness $  \mathbf{r}  $ as input of  the mechanism $M$  is in $ \{0, 1\}^*$.  For a fixed outcome $z$, $M$ needs only a finite number of coins $n = n(D, f, z)$ to determine whether $M(D, f) = z$.  Therefore,
we assume that  if  $ M(D,f; \mathbf{r}) = z   $ where $ \mathbf{r} \in \{0, 1\}^n$, then for any $\mathbf{r}'$ with $\mathbf{r}$
as its prefix, we still have $ M(D, f;  \mathbf{r}')=z$, that is, providing $M$ with extra coins does not change its output, which is
consistent with the model in \cite{DLMV12}.
...
We define the $\delta$-Santha-Vazirani source $\mathcal{SV}(\delta)$ to be the set of all-Santha-Vazirani distributions.
For a distribution $SV(\delta) \in \mathcal{SV}(\delta)$,  we define $SV(\delta, n)$  as the distribution $SV(\delta)$
restricted to the first $n$ coins $ x_1 x_2  \ldots x_n$. We let $\mathcal{SV}(\delta, n)$ to be the set of all distributions $SV(\delta, n)$.
}
...
Every distribution over $\{0, 1\}^n$ generated from $\mathcal{BCL}(\delta, b, n)$ is called  a
 $(\delta, b, n)$-BCL distribution  BCL$(\delta, b, n)$.
...
family of functions. A mechanism $M$ is $(\mathcal{R}, \xi)$-differentially private for  $\mathcal{F}$ if for all neighboring databases  $D_1, D_2 \in \mathcal{D}$, all $f \in \mathcal{F}$,  all possible outputs $z \in \mathbb{Z}$, and  all  distributions $R \in   \mathcal{R}$:
  $$ \frac{\Pr[M(D_1, f; \mathbf{r})=z  ]}{ \Pr[M(D_2, f; \mathbf{r})=z ]}  \leq 1+ \xi.$$
...
In this paper we employ the upper bound of the ratio of probabilities introduced in \cite{DLMV12} other than the traditional upper bound ``$e^{\xi}$''
...
we have $ e^{\xi} \approx 1+ \xi$, and when $ \xi \geq 0$, we always have $1+ \xi \leq e^{\xi}$.
 ...
 Denote $ T(D, f, z )  \overset{def}{=} \{  \mathbf{r} \in \{0, 1\}^n \mid z = M(D, f; \mathbf{r}) \}$ as   the set of all coins   $\mathbf{r} $
such that $M$ outputs $z$ when  running on  database $D$, query $f$, and randomness $\mathbf{r}$.
...
Then we observe
that these concepts are sufficient to design finite-precision accurate and differentially private mechanisms based on  BCL and SV sources.


For a bit sequence $\mathbf{x} =x_1, \ldots,  x_m \in \{0, 1\}^m$, denote $ \textmd{SUFFIX}(\mathbf{x}) \overset{def}{=} \{  \mathbf{y} = y_1, y_2, \ldots \in \{0, 1\}^* \mid x_i = y_i~for~all~i \in [m]\}$ as the set of all bit strings that have $\mathbf{x}$ as a prefix.  Define $ \textmd{SUFFIX}(\mathbf{x}, n)
 \overset{def}{=} \textmd{SUFFIX}(\mathbf{x}) \cap \{0, 1\}^n$.

Assume that $z = \frac{k}{\varepsilon} \in \mathbb{Z}$, where $k \in \mathbb{Z}$.   Denote $n \overset{def}{=} n( \min (f(D_1), f(D_2)), k)$.
...
Recall that the concept of SV-consistent sampling \cite{DLMV12} is as follows.
\vspace{0.15cm}
...
\begin{theorem}
\label{CSDP}
  {\slshape  If Mechanism $M$ is a  compact $(\zeta, c)$-BCL-consistent sampling mechanism for $(\delta, b, n)$-BCL-sources,  then $M$ is $(\mathcal{BCL}(\delta, b, n), \xi)$-differentially private,
...
Then by Theorems 1 and 2, the mechanism here and the modified mechanism of \cite{DLMV12} are  $(\mathcal{BCL}(\delta, b, n), \xi)$-differentially private and $( \mathcal{SV}(\delta), \xi')$-differentially private, where  $\xi'$ is a specific $\xi$ by letting $b=0$.
...
Let $ \bar{s}_{y-1}(k-1)$ (resp. $ \bar{s}_{y-1}(k)$) be  $s_{y-1}(k-1)$ (resp. $s_{y-1}(k)$), rounded  to its  $n \overset{def}{=}n(y-1,  k)$ most significant figures.
 We round $s_{y}(k-1)$ (resp.  $s_{y}(k)$) to its $n \overset{def}{=}n(y-1, k)$  most significant figures. Assume the binary decimal representation of the rounded $s_{y}(k-1)$ (resp.  $s_{y}(k)$) is $0. 0 \ldots 0 r_1 r_2 \ldots r_n$ (resp.  $0.0 \ldots 0 q_1 q_2 \ldots q_n$), then let
 $ \bar{s}_{y}(k-1) = 0.0 \ldots 0 r_1 r_2 \ldots r_n + 0.0 \ldots 0 r'_1 r'_2 \ldots r'_n$ (resp. $\bar{s}_{y}(k) = 0.0 \ldots 0 q_1 q_2 \ldots q_n + 0.0 \ldots 0 q'_1 q'_2 \ldots q'_n$), where the position of $r_1$ after the binary point is the same as that of $r'_1$, $ r'_i = 0$ for $i \in  [n-1]$,  and  $r'_n=1$ (resp. the position of $q_1$ after the binary point is the same as that of $q'_1$,  $ q'_i = 0$ for $i \in  [n-1]$ and  $q'_n=1$).
...
\vspace{0.15cm}
 ...
 ${\textsf{Step~3}}$ {\slshape  Denote $\overline{Z}_y$ (resp. $\overline{Z}_{y-1}$) as the  output distribution of  $ \overline{M}^{   \textsf{CBCLCS} }_{\varepsilon}(D_1, f)$ (resp. $ \overline{M}^{   \textsf{CBCLCS} }_{\varepsilon}(D_2, f)$), which approximates  $Z_y$ (resp. $Z_{y-1}$).   Let  $ S_0 = \{ \mathbf{r} \in \{0,  1\}^n  \mid \Pr[ Y= \mathbf{r}] \neq 0   \}$, where $Y$ is a $(\delta, b, n)$-BCL distribution.
 ...
 We obtain distribution $\overline{Z}_y$ (resp. $\overline{Z}_{y-1}$) from $U_{ S_0}$
by sampling a sequence of bits $\mathbf{r}=r_1, r_2, \ldots, r_n \in \{0, 1\}^n$ (resp. $\mathbf{r}'=r'_1, r'_2, \ldots, r'_n \in \{0, 1\}^n$)
...
The lemma below is one core step to achieve consistent sampling. Though it has essentially been proved by Dodis et al. \cite{DLMV12},  in \cite{DLMV12} there  exist some typos and the upper bound is not tight. Hence, we modify the Lemma A.1 of \cite{DLMV12} and prove it for completeness.  The comparison
...
& = e^{\varepsilon } > 1\\
\end{align*}
for $\varepsilon  \in (0, 1)$,  we have  $n(y, k) \leq n(y-1,k)$. Therefore, $$ n \overset{def}{=} \max \{ n(y, k), n(y-1,k)\}=n(y-1,k).$$
...
\begin{lemma}
\label{f-inf}   {\slshape
 % Denote  $  n(y-1,k) \overset{def}{=} \log  \frac{1}{| I_{y-1}(k)|} + c_0$,
% and   $  n(y-1,k) \overset{def}{=} \log  \frac{1}{| I_{y-1}(k)|} + c_0$,
% where $c_0$ is a constant such that  $2^{c_0 -b } -2^{-b} \geq 1$.
  For all $y, k  \in \mathbb{Z}$,
we have
$$    | \bar{I}'_y(k) | \leq | I'_y(k)|, $$
$$    |  \bar{I}_{y-1}(k)| \geq |I_{y-1}(k)|- 2^{-n( y-1, k )},  $$
$$  |I_{y}(k)| + 2^{-n(y-1, k)}  \geq |  \bar{I}_{y}(k)| \geq |I_{y}(k)|- 2^{-n(y-1, k)}.  $$
 }
...
\begin{proof} Since $s_{y-1}(k-1) \geq  \bar{s}_{y-1}(k-1)$, and $\bar{s} _{y}(k-1) \geq s _{y}(k-1) - 2^{-n(y-1, k)} + 2^{-n(y-1, k)}$, we get $    | \bar{I}'_y(k) | \leq | I'_y(k)|$.

 Since $  \bar{s}_{y-1}(k) \geq s _{y-1}(k) - 2^{-n(y-1, k)}$ and $  \bar{s}_{y-1}(k-1) \leq s _{y-1}(k-1 )$,  we have  $ |  \bar{I}_{y-1}(k)| \geq |I_{y-1}(k)|- 2^{-n(y-1, k)}. $

 Since  $ \bar{s}_{y}(k) \geq s _{y}(k) - 2^{-n(y-1, k)} + 2^{-n(y-1, k)} $ and $  \bar{s}_{y}(k-1) \leq s _{y}(k-1) + 2^{-n(y-1, k)}$,  we have  $ |  \bar{I}_{y}(k)| \geq |I_{y}(k)|- 2^{-n(y-1, k)}. $

 Since $ \bar{s}_{y}(k) \leq   s _{y}(k) + 2^{-n(y-1, k)}$ and $  \bar{s}_{y}(k-1) \geq s_y(k-1)$, we have $|  \bar{I}_{y}(k)| \leq   |I_{y}(k)| + 2^{-n(y-1, k)}$.
...
\begin{lemma}
\label{ori}
{\slshape  Denote $ n(y-1,k) \overset{def}{=} \log  \frac{1}{| I_{y-1}(k)|} + \log(2^b+1)$.   For all $y, k  \in \mathbb{Z}$,
we have
$$ |\textmd{STR }( \bar{I'}_y (k), n(y-1,k) ) \cap S_0|  \leq   (2^{-b} +1) \cdot e \cdot \varepsilon,~  |\textmd{STR }( \bar{I}_{y-1}(k), n(y-1,k)) \cap S_0 | \geq 1.$$

\ignore{

$$|\textmd{STR }( \bar{I}_{y}(k), n(y-1,k)) \cap S_0 | \geq    e^\varepsilon  \cdot (2^{-b} +1) -2^{-b}. $$
...
\begin{proof}   Let  $n \overset{def}{=} n(y-1, k)$ for shorthand.
...
\label{CS}
  {\slshape    Denote $ n(y-1,k) \overset{def}{=} \log  \frac{1}{| I_{y-1}(k)|} + \log(2^b+1)$.     For all $y, k  \in \mathbb{Z}$,
we have $$ \frac{|\textmd{STR }( \bar{I'}_y (k), n(y-1,k)) \cap S_0 |}{|\textmd{STR }( \bar{I}_{y-1}(k), n(y-1,k)) \cap S_0 |} \leq  (2^b+1)  \cdot e \cdot \varepsilon. $$   }
...
\begin{theorem}
\label{BCLCS}
{\slshape  Denote $ n(y-1,k) \overset{def}{=} \log  \frac{1}{| I_{y-1}(k)|} + \log(2^b+1)$.  Let $\textbf{u}$ be the longest common prefix of all strings in  $ \bar{I} \overset{def}{=} \bar{I}_y(k) \cup  \bar{I}_{y-1}(k)$.  Then
$$|\textmd{SUFFIX}( \mathbf{u}, n(y-1,k) ) \cap S_0 |  \leq   \frac{e \cdot  (2^b+1) }{1-e^{-1}}.$$    }

\end{theorem}



\begin{proof}  For simplicity, let  $n \overset{def}{=} n(y-1, k)$.
...
\begin{theorem}
 ...
 compact $(   (  2^b+1 ) \cdot e \cdot \varepsilon,  \log (\frac{e \cdot (2^b+1) }{1-e^{-1}}) )$-BCL-consistent sampling mechanism for $(  \delta, b, n )$-BCL sources.
Therefore,  $\overline{M} ^{    \textsf{CBCLCS} }_{\varepsilon }$ is
$ (\mathcal{U}, 2 e \cdot \varepsilon)$-differentially private and $(\mathcal{BCL}( \delta, b, n ), \xi)$-differentially private  for  $ \xi =     (\frac{1+ \delta }{1- \delta})^{ \log ( \frac{e \cdot  ( 2^b+1  ) }{1-e^{-1}})  } \cdot (\frac{1+ \delta}{2})^{-b} \cdot  (  2^b+1 ) \cdot e \cdot \varepsilon$.
...
has    $( \mathcal{BCL} ( \delta, b, n), O(\frac{1}{\varepsilon} \cdot   \frac{1}{1- \delta}))$-utility and $(\mathcal{U}, O(\frac{1}{\varepsilon}))$-utility.
     }

\end{theorem}


\begin{proof}  For any $BCL(\delta, b, n)$ distribution, we calculate the  upper bound of $$ \mathbb{E}_{ \textbf{r} \leftarrow  BCL (\delta, b, n) }
[| \overline{M} ^{  \textsf{CBCLCS} }_{\varepsilon } (D, f; \textbf{r}) - f(D) |]$$ as follows.
Let $f(D)=y$. Then

\begin{align*}
& \mathbb{E}_{ \textbf{r} \leftarrow BCL(\delta, b, n) } [|  \overline{M} ^{    \textsf{CBCLCS} }_{\varepsilon }(D, f; \textbf{r})-y|]\\
& = \sum \limits_{k = - \infty}^{\infty} \Pr \limits_{ \textbf{r} \leftarrow  BCL (\delta, b, n) } [  \overline{M} ^{    \textsf{CBCLCS} }_{\varepsilon }(D, f; \textbf{r}) = \frac{k}{ \varepsilon}] \cdot | \frac{k}{ \varepsilon} - y|.\\
\end{align*}


Let $\textbf{a}$ be the longest common  prefix of all strings in $\textmd{STR } (\bar{I}_y(k), n)$. Denote $I_0  \overset{def}{=} \textmd{SUFFIX} (\textbf{a}0, n) \cap \textmd{STR} (\bar{I}_y(k), n)$ and $I_1 \overset{def}{=} \textmd{SUFFIX} (\textbf{a}1, n) \cap \textmd{STR} (\bar{I}_y(k), n)$. Thus, $I_0 \cup I_1 =  \textmd{STR} (\bar{I}_y(k), n)$.
Correspondingly,  we have
$$ \Pr \limits_{ \textbf{r} \leftarrow BCL(\delta, b, n) } [  \overline{M} ^{    \textsf{CBCLCS} }_{\varepsilon }(D, f; \textbf{r}) = \frac{k}{\varepsilon}] \leq ( \frac{1+ \delta}{2})^{|\textbf{a}0 |}  + ( \frac{1+ \delta}{2})^{|\textbf{a}1 |} \leq 2 \cdot ( \frac{1+ \delta}{2})^{  \log ( \frac{1}{ | \bar{I}_y(k)|}  )}.$$
...
\end{proof}
 ...
 By Lemma \ref{f-inf},  $ |\bar{I}_y(k)| \leq |I_y(k)| + 2^{-n(k-1,y)} = |I_y(k)| +    \frac{1}{2^b+1}   |I_{y-1}(k)|$.  Hence,
...
$( \mathcal{BCL} ( \delta, b, n),   O(\frac{1}{\varepsilon} \cdot   \frac{1}{1- \delta}))$-utility and $(\mathcal{U}, O(\frac{1}{\varepsilon}))$-utility.
...
\begin{theorem}
\label{spe}
  {\slshape  There exists an explict   $(\mathcal{BCL}(\delta, b, n), \xi)-$differentially private and $(U_n, \rho)$-accurate mechanism M for the
...
{\slshape   If     $b   \geq \frac{  \log ( \xi \rho )  + 9 }{  \log(1+ \delta)} = \Omega (  \frac{\log( \xi \rho)+1}{ \delta})$,    then no $( \mathcal{BCL}(\delta, b, n), \xi  )-$differentially private and $(U_n, \rho)$-accurate mechanism for the Hammimg weight queries exists.          }
\end{theorem}
...
\begin{corollary}
\label{gen}
  {\slshape  Assume that the mechanism M is  $( \mathcal{BCL}(\delta, b, n), \xi)-$differentially private and $(U_n, \rho)$-accurate for the
...
Corollary \ref{gen}  implies that  it's possible to construct a   $( \mathcal{BCL}(\delta, b, n), \xi)-$differentially private and $(U_n, \rho)$-accurate mechanism for  Hammimg weight queries, where $ \rho > \frac{2^{b \cdot \log (1+ \delta) -9}}{\xi}$.  In this paper, we show an explicit  construction of such mechanisms.
...
We would like to thank  Yevgeniy Dodis,  Adriana  L\'{o}pez-Alt,  and  Frank Mcsherry for helpful discussions. In particular, we thank Prof. Yevgeniy Dodis for proposing the project ``Do differential privacy with BCL sources for reasonably high b''.
