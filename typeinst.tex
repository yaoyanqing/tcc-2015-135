...
denote the operation of sampling a random   $\mathbf{r}$  according to $R$.  For a set $S$, we write $U_S$ to denote the uniform distribution over $S$. Let $\{0, 1\}^* \overset{def}{=} \bigcup \limits_{ m \in \mathbb{Z}^+ } \{0, 1\}^m$.
...
We call a family   $ \mathcal{R}$ of distributions over $\{0, 1\}^*$ a source.       Denote $ \mathcal{U} $  as the uniform source, which
 is the set containing only the distribution $U$  over $\{0, 1\}^*$ that samples each bit independently and uniformly at random.
...
\begin{remark} In this paper,   we assume that the   random coins  employed by $M$  is in $\{0, 1\}^*$, that is,  though $M$ has at its disposal a
possibly infinite number of random bits,   for a fixed outcome $z  \in \mathbb{Z}$, $M$ needs only a finite number
...
Consider a mechanism $M$ with randomness space $\{0, 1\}^*$. For all neighboring databases  $D_1, D_2 \in \mathcal{D}$, denote $ \widetilde{T}_i(D_i, f, z)  \overset{def}{=} \{  \mathbf{r} \in \{0, 1\}^* \mid z = M(D_i, f; \mathbf{r}) \}$, where $i \in \{1, 2\}$,   as  the set of all coins  $\mathbf{r} \in \{0, 1\}^*$ such that $M$ outputs $z$ when running on  database $D_i$, query $f$, and random coins $\mathbf{r}$.
